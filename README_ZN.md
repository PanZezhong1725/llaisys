...
在 InfiniLM 中支持新的模型。项目应以适配英伟达 GPU 平台为基础；在此基础上，若同时支持国产加速平台，可根据适配难度获得额外加分。其中，昇腾平台的实现差异最大，因此可获得最多的额外加分。评判还将综合考虑模型复杂度、引入的可复用基础设施，以及实现和测试的完整程度。对于需要引入 MLA、MTP、MoE、NSA、Mamba、RWKV、UltraMem、Titans、MiniMax 等新架构或机制的模型，其工作量与几乎完全复用现有实现的模型变体将区别评估。
