...
void setDevice(int device) { check(cudaSetDevice(device), "cudaSetDevice"); }
void deviceSynchronize() { check(cudaDeviceSynchronize(), "cudaDeviceSynchronize"); }
...
void destroyStream(llaisysStream_t stream) {
    if (stream != nullptr) check(cudaStreamDestroy(reinterpret_cast<cudaStream_t>(stream)), "cudaStreamDestroy");
}
...
void freeDevice(void *ptr) { if (ptr != nullptr) check(cudaFree(ptr), "cudaFree"); }
void *mallocHost(size_t size) {
    void *ptr = nullptr;
    check(cudaMallocHost(&ptr, size), "cudaMallocHost");
    return ptr;
}
void freeHost(void *ptr) { if (ptr != nullptr) check(cudaFreeHost(ptr), "cudaFreeHost"); }
...
void memcpySync(void *dst, const void *src, size_t size, llaisysMemcpyKind_t kind) {
    check(cudaMemcpy(dst, src, size, memcpyKind(kind)), "cudaMemcpy");
}
void memcpyAsync(void *dst, const void *src, size_t size, llaisysMemcpyKind_t kind, llaisysStream_t stream) {
    check(cudaMemcpyAsync(dst, src, size, memcpyKind(kind), reinterpret_cast<cudaStream_t>(stream)), "cudaMemcpyAsync");
}
