 ...
 */
namespace {
struct IluvatarThreadResources {
    // （设备 ID, cuBLAS 句柄）
    std::unordered_map<int, cublasHandle_t> cublas_handles;

    ~IluvatarThreadResources() {
...
thread_local IluvatarThreadResources resources;

} // namespace

cublasHandle_t getCublasHandle() {
    int device = 0;
    ILUVATAR_CUDA_CHECK(cudaGetDevice(&device));  // 获取当前线程绑定的 GPU ID
    ...
    ILUVATAR_CUBLAS_CHECK(cublasCreate(&handle));
