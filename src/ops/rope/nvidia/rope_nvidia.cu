...
__global__ void rope_kernel(T *out, const T *in, const int64_t *pos_ids, float theta,
    ...
    // Compute cos/sin on device to match Torch's GPU computation.
    // freq = pos / (theta ** (2*j/head_dim))
    // Torch computes theta ** exponent using double precision (theta is a Python
    // float/double), then casts the result back to float32. Use double here to
    // match Torch's precision.
    ...
    // Match Torch's computation: x_a * cos - x_b * sin, x_b * cos + x_a * sin
